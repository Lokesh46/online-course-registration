...
<body>
    <div class="main-container d-flex">
        ...
        <div class="table-wrapper">
            ...
            $sql = "SELECT Faculty_Id, Name,dept_name FROM faculty,department where department.dept_Id=faculty.dept_id";
            ...
            // Otherwise fetch all the rows returned by the query one by one
                ...
                if ($queryResult->num_rows > 0) {
                    while ($rowObj = $queryResult->fetch_object()) {
                        echo "<tr>
                            <td>{$rowObj->Faculty_Id}</td>
                            <td>{$rowObj->Name}</td>
                            <td>{$rowObj->dept_name}</td>
                            <td><a style='color: black' id='delete' href=del.php?id={$rowObj->Faculty_Id}><i class='fas fa-trash'></a></td>";
                    }
                }
            }
            ?>
